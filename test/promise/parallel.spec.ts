...
describe('parallel', () => {
  it('should execute tasks in parallel', async () => {
    ...
    const results = await parallel([() => delay(50, 'a'), () => delay(50, 'b'), () => delay(50, 'c')]);
  ...
  it('should respect concurrency limit', async () => {
    const start = Date.now();
    const results = await parallel([() => delay(50, 'a'), () => delay(50, 'b'), () => delay(50, 'c')], 2);
    const elapsed = Date.now() - start;
    expect(results).toEqual(['a', 'b', 'c']);
  ...
  it('should preserve order', async () => {
    const results = await parallel([() => delay(100, 'first'), () => delay(50, 'second'), () => delay(150, 'third')]);
    expect(results).toEqual(['first', 'second', 'third']);
  });

  it('should handle synchronous tasks', async () => {
    const results = await parallel([() => 'a', () => 'b', () => 'c']);
    expect(results).toEqual(['a', 'b', 'c']);
  });

  it('should reject on first error', async () => {
    const tasks = [() => delay(50, 'a'), () => Promise.reject(new Error('error in task 2')), () => delay(50, 'c')];
    await expect(parallel(tasks)).rejects.toThrow('error in task 2');
  });

  it('should handle concurrency of 1 (sequential)', async () => {
    const start = Date.now();
    const results = await parallel([() => delay(50, 'a'), () => delay(50, 'b'), () => delay(50, 'c')], 1);
    const elapsed = Date.now() - start;
    expect(results).toEqual(['a', 'b', 'c']);
