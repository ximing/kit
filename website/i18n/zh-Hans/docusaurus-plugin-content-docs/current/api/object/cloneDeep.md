...
console.log(original.b.d.e); // 2 (原始对象未改变)
...
console.log(arr[2][2][1]); // 6 (原始数组未改变)
...
console.log(clonedObj !== obj); // true (独立副本)
...
const complex = {
  users: [
    { id: 1, name: '张三', tags: ['开发', '设计'] },
    { id: 2, name: '李四', tags: ['开发'] },
  ],
  metadata: {
...
clonedComplex.users[0].tags.push('经理');
clonedComplex.metadata.tags.add('已归档');

console.log(complex.users[0].tags); // ['开发', '设计']
console.log(clonedComplex.users[0].tags); // ['开发', '设计', '经理']
```
...
console.log(restored.nested.x); // 10 (独立副本)
```
