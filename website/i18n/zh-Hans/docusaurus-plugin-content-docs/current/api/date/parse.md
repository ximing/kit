...
id: parse
title: parse
description: '按照给定的格式字符串解析日期字符串并返回 Date 对象'
---

# `parse`

按照给定的格式字符串解析日期字符串并返回 Date 对象。该函数支持多种格式 token 来解析各种日期字符串格式。
...
function processApiResponse(apiData: string) {
  ...
  if (!date) {
    throw new Error('来自 API 的日期格式无效');
  }
  return date;
}
    ...
    <div style={{ padding: '20px', background: '#f5f5f5', borderRadius: '8px' }}>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        <input
          ...
          placeholder="例如: 2024-01-15 14:30:45"
        />
      </div>
      <div style={{ marginBottom: '15px' }}>
